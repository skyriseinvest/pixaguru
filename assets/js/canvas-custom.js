(function( $ ) {
	...
	setTimeout(function(){$('.pixaguru-loader').hide();},1000);
	...
	var currentBgImage = "";
	var properties_to_save = Array("width", "height", "orig_src", "clip_left", "clip_top", "clip_width", "clip_height", "is_frame", "id","max");
	...
	if (screenWidth < 600) {
		...
		var crop_img_corners_size = 14;
		if (Modernizr.touch) {
			canvas_corners_size = 14;
			crop_img_corners_size = 16;
		}
	}else{
	...
	canvas.on("selection:created", function(e) {
		...
		localStorage.setItem('selectType',activeObj.type);
	  if(activeObj.type === "group") {		
		...
		e.target.forEachObject(function(obj) {
			...
			$('#clr-color-marker').mouseup(function(e) {	
				var color = $('.ed_group_colorPicker').val(); 
				$('.ed_group_colorPicker').next().css({
	...
	$(function() {
		...
		$(document).on('click','#save_template',function(e){
			...
			$.ajax({
				...
				success: function(data){
					...
					setTimeout(function(){$('.pixaguru-loader').hide();},2000);
				}
			});
		});
		...
		$('.get_canvas_image').on('click',function(e){	
			$('.btn-close').click();
			setTimeout(function(){$('.pixaguru-loader').show();},200);				
		    var b = canvas.width;
	...
	function call_api_pixabay(q, obj, cls, page){
		if(YOUTUBEKEY !="null"){
			...
			$('.pixabayAPIError').text('API Key Not Available Please Add pixabay API Key In Admin Panel');
			$.toaster('API Key Not Available Please Add pixabay API Key In Admin Panel','Error');
			setTimeout(function(){$('.pixabayAPIError').empty();},5000);
			return false;
		}
	}
	...
	function bootstrap_colorpicker_js( where = null ){
		...
		if(where == null){			
			...
			Coloris({
				el: '.pg_colorPicker',
				swatches: [
					'#FF0000','#FFFFFF','#00FFFF','#C0C0C0',
					'#0000FF','#808080','#000000','#ADD8E6',
			...
			$('.pg_colorPicker').on("click",function(e) {
				var index =$(this).parents('.pg-colorPicker-container').data('index');
			...
			$('#clr-color-marker').mouseup(function(e) {
				var color = $('.clr-open').find('.clr-color').val();
				
				var index = localStorage.getItem('index');
				
				if(index == 'null'){
					index = $(this).parents('.pg-colorPicker-container').data('index');
				}
				...
				console.log(index);
				if(canvas.getActiveObject()!=null){
					if(index != null && index != 'undefined' && canvas.getActiveObject().paths.length){
						var obj = canvas.getActiveObject().paths[index];
						obj.set({
							fill : color
						});	
		...
		var xhr = $.ajax({
			...
			success: function(data){
				...
				if(result.success){
					...
					currentBgImage = result.image_url;
					...
					$('.upoadFileError').text('Image size you added is greater than allowed uppload size. Allowed size is of 8 mb');
					setTimeout(function(){$('.upoadFileError').empty()},3000);
				}
			}
		});
	} 
	...
	/**
	  * Remove Background
	  */
	function canvas_add_transparent(url) {	
		...
		if (canvas.getActiveObject() && "image" == canvas.getActiveObject().type) {
		  ...
		  j.onload = function () {
			var p = new fabric.Image(j);
			var m = j.width;
			var l = canvas.getActiveObject().width;
			var n = 1;
			var w = canvas.getActiveObject().width;
			var h = canvas.getActiveObject().height;
			if (j.width > w || j.height > h) {
			  j.width > j.height ? (m = j.width) : (m = j.height);
			  w > h ? (l = w) : (l = h);
			  n = l / m;
	 ...
	 $(document).on("click", ".remove_template_bg", function () {
		canvas.backgroundColor = "";
		canvas.renderAll();
		save_history(1);		
	  });
	$(document).on('click',"#remove-bg-content, #remove-image-bg-content",function (d) {
		d.preventDefault();		
		var tp = $(this).attr("data-type");
		// $.toaster('This service has been closed for now','Error');	
		// return
		$('.pixaguru-loader').show();
		...
		if (currentBgImage) {			
			...
			success: function (data) {				
				var result = jQuery.parseJSON(data);
				if(result.status==1){
					$('.pixaguru-loader').hide();
					if (tp == "image") {
						canvas_add_transparent(result.url);
					} else {
						$('.pixaguru-loader').hide();
						edit_bg_crop_image(result.url);
						set_canvas_bg(result.url);
					}
				}else{
					$.toaster(result.msg,'Error');
				}
			},
			});
		}
